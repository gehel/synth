...
def render_ascii(panel: Panel) -> str:
    ...
    _draw_border(grid, total_rows, total_cols)

    for pc in placed:
        _draw_component(pc, grid, total_rows, total_cols)

    return "\n".join("".join(row) for row in grid)


def _draw_border(grid: list[list[str]], total_rows: int, total_cols: int) -> None:
